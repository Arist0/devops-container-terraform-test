...
module "lookup-security-groups" {
  source = "./infrastructure/modules/aws/lookups/security_groups"
  names = [
...
module "ecs-cluster" {
  source = "./infrastructure/modules/aws/ecs"
  app_name = local.app_name
  environment = local.environment
}

module "logs" {
  source = "./infrastructure/modules/aws/cloudwatch"
  app_name = local.app_name
  environment = local.environment
  resource = "ecs"
  retention_in_days = 7
...
module "ecs-lb" {
  source = "./infrastructure/modules/aws/lb/standard"
  name = local.app_name
  environment = local.environment
  vpc_id = local.vpc_id
